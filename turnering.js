...
export async function startPuljespill(turneringId) {
  ...
  const puljeMedKamper = (() => {
    ...
    // Bruk global rotasjon for 4 puljer / 6 baner — krever nøyaktig 4 lag per pulje
    if (antallPuljer === 4 && antallBaner === 6 && minLagPerPulje === 4) {
      return genererKamperMed4Puljer6Baner(t.puljer);
    }
    // Bruk global rotasjon for 3 puljer / 6 baner — krever 5 lag per pulje
    if (antallPuljer === 3 && antallBaner === 6 && minLagPerPulje >= 5) {
      return genererKamperMed3Puljer6Baner(t.puljer);
    }
    // Bruk global rotasjon for 2 puljer / 6 baner — krever 6 lag per pulje
