...
import {
  ...
  getErAdmin, setErAdmin, registrerPinGetter,
} from './admin.js';
// ════════════════════════════════════════════════════════
// KLUBB-KONFIGURASJON
// ════════════════════════════════════════════════════════
...
function lyttPaaSpillere() {
  ...
  spillerLytterAvmeld = onSnapshot(
    ...
    (snap) => {
      ...
      visSpillere();
      if (_sisteDeltakereApen) oppdaterSisteDeltakereInPlace();
    },
    (feil) => {
      document.getElementById('spiller-laster').style.display = 'none';
...
function visSisteDeltakere() {
  ...
  const uvalgte = spillere.filter(s => !aktiveIds.has(s.id) && !ventendeIds.has(s.id));
  const valgte  = spillere.filter(s =>  aktiveIds.has(s.id) ||  ventendeIds.has(s.id));
  ...
  if (valgte.length > 0) {
    ...
    html += valgte.map(s => lagSpillerHTML(s, aktiveIds.has(s.id), ventendeIds.has(s.id))).join('');
  }
...
function visSokIPanel(q) {
  ...
  const treff = (app.spillere ?? [])
    .filter(s => (s.navn ?? '').toLowerCase().includes(qLow) || aktiveIds.has(s.id) || ventendeIds.has(s.id))
    .sort((a, b) => {
      const aValgt = (aktiveIds.has(a.id) || ventendeIds.has(a.id)) ? 1 : 0;
      const bValgt = (aktiveIds.has(b.id) || ventendeIds.has(b.id)) ? 1 : 0;
      if (aValgt !== bValgt) return aValgt - bValgt;
...
let _forrigeVisResultatTeller = 0;
...
function startLyttere() {
  if (!db || !app.treningId) return;
  _forrigeVisResultatTeller = 0;

  const l1 = onSnapshot(
    doc(db, SAM.TRENINGER, app.treningId),
    snap => {
      // ── Punkt 1: Økt slettet (admin avbrøt) — send alle til hjemskjermen ──
      if (!snap.exists()) {
        stoppLyttere();
        app.treningId    = null;
        app.baneOversikt = [];
        ...
        naviger('hjem');
        visMelding('Økten ble avbrutt av administrator.');
        return;
      }
      ...
      if (data.status === 'avsluttet') {
        ...
        stoppLyttere();
        naviger('slutt');
        // Start klubb-lytter så brukere automatisk følger med når admin starter ny økt
        if (!getErAdmin()) startKlubbLytter();
        return;
      }

      // ── Punkt 3: Admin trykket «Neste runde» — vis runderesultat ──
      // Bruker en teller (visResultatTeller) som økes ved hver ny runde-visning
      // slik at lytteren alltid fanger endringen (boolean ville ikke trigget ved gjenoppkobling)
      const nyTeller = data.visResultatTeller ?? 0;
      const forrigeTeller = _forrigeVisResultatTeller ?? 0;
      _forrigeVisResultatTeller = nyTeller;

      // ── Punkt 4: Ny runde startet — alltid prioritert over resultatliste ──
      ...
      // ── Punkt 3: Admin viser resultatliste ──
      if (!getErAdmin() && nyTeller > forrigeTeller) {
        visRundeResultat();
        return;
      }
    },
...
// ════════════════════════════════════════════════════════
// HJEM-LYTTER — oppdaterer hjemskjermen hvis admin avslutter
// ════════════════════════════════════════════════════════
let _hjemLytterAvmeld  = null;
let _klubbLytterAvmeld = null;

// ── Hjem-lytter: brukes når bruker er på hjemskjermen med aktiv økt ──
// Oppdaterer hjemskjermen hvis admin avslutter økten.
function startHjemLytter() {
  if (!db || !app.treningId) return;
  stoppHjemLytter();
  _hjemLytterAvmeld = onSnapshot(
    doc(db, SAM.TRENINGER, app.treningId),
    snap => {
      ...
      if (data.status === 'avsluttet') {
        ...
        sessionStorage.removeItem('aktivTreningId');
        visHjemStatus();
        visMelding('Økten er avsluttet av administrator.');
      }
    },
    feil => console.warn('[HjemLytter]', feil?.message ?? feil)
  );
}

function stoppHjemLytter() {
  if (_hjemLytterAvmeld) { try { _hjemLytterAvmeld(); } catch (_) {} _hjemLytterAvmeld = null; }
}

// ── Klubb-lytter: brukes etter avsluttet økt ──
// Lytter etter ny aktiv økt på klubbnivå.
// Når admin starter ny økt, sendes alle brukere direkte til baneoversikten.
function startKlubbLytter() {
  if (!db || !aktivKlubbId) return;
  stoppKlubbLytter();
  _klubbLytterAvmeld = onSnapshot(
    query(
    ...
    async snap => {
      ...
      const nyOkt = snap.docs[0];
      const treningId = nyOkt.id;
      // Ikke reager på den samme økten vi allerede kjenner til
      if (treningId === app.treningId) return;
      stoppKlubbLytter();
      const ok = await gjenopprettTrening(treningId);
      if (ok) visMelding('Ny økt startet!');
    },
    feil => console.warn('[KlubbLytter]', feil?.message ?? feil)
  );
}

function stoppKlubbLytter() {
  if (_klubbLytterAvmeld) { try { _klubbLytterAvmeld(); } catch (_) {} _klubbLytterAvmeld = null; }
}
...
function oppdaterKampStatus(kamper) {
  ...
  // Oppdater poengskjermen in-place hvis den er åpen
  oppdaterPoengskjermFraCache();
}

/**
 * Oppdaterer inputfelt på poengskjermen med nye verdier fra Firestore,
 * men bare for felt som ikke er i fokus — unngår å forstyrre aktiv inntasting.
 */
function oppdaterPoengskjermFraCache() {
  // Sjekk om poengskjermen faktisk vises — bruk synlighet i stedet for klasse
  const poengSkjerm = document.getElementById('skjerm-poeng');
  if (!poengSkjerm) return;
  const rect = poengSkjerm.getBoundingClientRect();
  if (rect.width === 0 && rect.height === 0) return;
  if (app.aktivBane == null) return;

  const bane = (app.baneOversikt ?? []).find(b => b.baneNr === app.aktivBane);
  if (!bane) return;

  const erSingel   = bane?.erSingel === true || (bane?.spillere?.length === 2);
  const erDobbel6  = app.er6SpillerFormat && (bane?.erDobbel === true);
  const parter     = erMix()
    ? [{ nr: 1, lag1: [0,1], lag2: [2,3] }]
    : (erSingel ? PARTER_6_SINGEL : (erDobbel6 ? PARTER_6_DOBBEL : getParter(bane?.spillere?.length ?? 4)));

  parter.forEach((par, i) => {
    const k = kampStatusCache[`bane${app.aktivBane}_${par.nr}`];
    if (!k?.ferdig) return;

    const el1 = document.getElementById(`s${i}_l1`);
    const el2 = document.getElementById(`s${i}_l2`);
    const statusEl = document.getElementById(`kamp-status-${i}`);
    const kortEl   = document.getElementById(`kk-${i}`);

    // Oppdater kun felt som ikke er i fokus
    if (el1 && document.activeElement !== el1 && el1.value !== String(k.lag1Poeng)) {
      el1.value = k.lag1Poeng;
      if (el1.classList.contains('ugyldig')) el1.classList.remove('ugyldig');
    }
    if (el2 && document.activeElement !== el2 && el2.value !== String(k.lag2Poeng)) {
      el2.value = k.lag2Poeng;
      if (el2.classList.contains('ugyldig')) el2.classList.remove('ugyldig');
    }

    // Oppdater status-merket
    if (statusEl) {
      statusEl.className = 'kamp-status lagret';
      statusEl.textContent = '✓ Lagret';
    }
    if (kortEl) kortEl.classList.remove('ugyldig');
  });

  // Oppdater Neste-knappen
  oppdaterPoengNav();
}
...
function visBaner() {
  ...
  if (redigerKnapp) redigerKnapp.style.display = (app.treningId && !erMix()) ? 'inline-flex' : 'none';
...
function visNesteRundeModal() {
  krevAdminMedDemo(
    ...
    async () => {
      ...
      // Konkurranse: tilby manuelt eller automatisk oppsett
      const tittelEl = document.getElementById('modal-velg-oppsett-tittel');
      const tekstEl  = document.getElementById('modal-velg-oppsett-tekst');
      if (tittelEl) tittelEl.textContent = erSiste ? 'Avslutte runden?' : 'Neste runde?';
      if (tekstEl)  tekstEl.textContent  = erSiste
        ? `Runde ${app.runde} er siste runde. Vil du se resultatene?`
        : `Runde ${app.runde} av ${app.maksRunder} er ferdig. Velg oppsett for neste runde:`;

      const manueltKnapp = document.querySelector('#modal-velg-oppsett .knapp-omriss[onclick*="manuelt"]');
      if (manueltKnapp) manueltKnapp.style.display = erSiste ? 'none' : 'flex';

      document.getElementById('modal-velg-oppsett').style.display = 'flex';
    }
  );
}
...
let _velgOppsettModus = null;

function velgOppsettModus(modus) {
  document.getElementById('modal-velg-oppsett').style.display = 'none';
  _velgOppsettModus = modus;
  if (modus === 'manuelt') {
    apneManueltOppsett();
  } else {
    visRundeResultat();
...
window.velgOppsettModus = velgOppsettModus;

function apneManueltOppsett() {
  const innhold = document.getElementById('manuelt-oppsett-innhold');
  const feilEl  = document.getElementById('manuelt-oppsett-feil');
  if (feilEl) { feilEl.style.display = 'none'; feilEl.textContent = ''; }

  const alleSpillere = [
    ...(app.baneOversikt ?? []).flatMap(b => b.spillere ?? []),
    ...(app.venteliste   ?? []),
  ];
  const antallBaner = app.baneOversikt.length;

  let html = '';
  for (let b = 0; b < antallBaner; b++) {
    const baneNr  = b + 1;
    const gjeldende = (app.baneOversikt[b]?.spillere ?? []).map(s => s.id);

    html += `<div style="margin-bottom:18px">
      <div style="font-family:'Bebas Neue',cursive;font-size:22px;letter-spacing:2px;color:var(--accent);margin-bottom:8px">
        Bane ${baneNr}
      </div>`;

    for (let p = 0; p < 4; p++) {
      const valgId = gjeldende[p] ?? '';
      html += `<div style="margin-bottom:8px">
        <div style="font-size:13px;text-transform:uppercase;letter-spacing:1px;color:var(--muted2);margin-bottom:4px">Spiller ${p + 1}</div>
        <select id="manuell-bane${baneNr}-spl${p}"
          style="width:100%;padding:12px 14px;background:var(--navy3);border:1px solid var(--border2);border-radius:12px;color:var(--white);font-family:'DM Sans',sans-serif;font-size:17px;appearance:none;-webkit-appearance:none;outline:none"
          onchange="oppdaterManuellStatus()">
          <option value="">— Velg spiller —</option>
          ${alleSpillere.map(s => `<option value="${escHtml(s.id)}" ${s.id === valgId ? 'selected' : ''}>${escHtml(s.navn ?? 'Ukjent')} (⭐${s.rating ?? 1000})</option>`).join('')}
        </select>
      </div>`;
    }
    html += `</div>`;
  ...
  html += `<div style="margin-bottom:16px">
    <div style="font-size:13px;text-transform:uppercase;letter-spacing:1.5px;color:var(--orange);font-weight:700;margin-bottom:6px">
      Venteliste (tildeles automatisk)
    </div>
    <div id="manuell-venteliste-vis" style="font-size:15px;color:var(--muted2);padding:8px 0">Ingen på venteliste</div>
  </div>`;

  innhold.innerHTML = html;
  oppdaterManuellStatus();
  document.getElementById('modal-manuelt-oppsett').style.display = 'flex';
}
window.apneManueltOppsett = apneManueltOppsett;

function fyllAutomatiskOppsett() {
  const alleSpillere = [
    ...(app.baneOversikt ?? []).flatMap(b => b.spillere ?? []),
    ...(app.venteliste   ?? []),
  ].sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));
  ...
  const antallBaner = app.baneOversikt.length;
  const aktive      = alleSpillere.slice(0, antallBaner * 4);

  for (let b = 0; b < antallBaner; b++) {
    for (let p = 0; p < 4; p++) {
      const el = document.getElementById(`manuell-bane${b + 1}-spl${p}`);
      if (el) el.value = aktive[b * 4 + p]?.id ?? '';
    }
  }
  oppdaterManuellStatus();
}
window.fyllAutomatiskOppsett = fyllAutomatiskOppsett;

function oppdaterManuellStatus() {
  const antallBaner  = app.baneOversikt.length;
  const alleSpillere = [
    ...(app.baneOversikt ?? []).flatMap(b => b.spillere ?? []),
    ...(app.venteliste   ?? []),
  ...
  for (let b = 0; b < antallBaner; b++) {
    for (let p = 0; p < 4; p++) {
      ...
      const val = el?.value ?? '';
      if (!val) continue;
      if (valgteIds.has(val)) duplikater.add(val);
      valgteIds.add(val);
    }
  }

  const vlSpillere = alleSpillere.filter(s => !valgteIds.has(s.id));
  const vlEl = document.getElementById('manuell-venteliste-vis');
  if (vlEl) {
    vlEl.textContent = vlSpillere.length > 0
      ? vlSpillere.map(s => s.navn ?? 'Ukjent').join(', ')
      : 'Ingen på venteliste';
    vlEl.style.color = vlSpillere.length > 0 ? 'var(--orange)' : 'var(--muted2)';
  }
  ...
  for (let b = 0; b < antallBaner; b++) {
    for (let p = 0; p < 4; p++) {
      const el  = document.getElementById(`manuell-bane${b + 1}-spl${p}`);
      const val = el?.value ?? '';
      if (el) el.style.borderColor = (val && duplikater.has(val)) ? 'var(--red2)' : 'var(--border2)';
    }
  }
}
window.oppdaterManuellStatus = oppdaterManuellStatus;

function lukkManueltOppsett() {
  document.getElementById('modal-manuelt-oppsett').style.display = 'none';
}
window.lukkManueltOppsett = lukkManueltOppsett;

async function bekreftManueltOppsett() {
  const antallBaner  = app.baneOversikt.length;
  const alleSpillere = [
    ...(app.baneOversikt ?? []).flatMap(b => b.spillere ?? []),
    ...(app.venteliste   ?? []),
  ];
  const spillerMap = Object.fromEntries(alleSpillere.map(s => [s.id, s]));

  const feilEl = document.getElementById('manuelt-oppsett-feil');
  const visFeil = (tekst) => {
    if (feilEl) { feilEl.textContent = tekst; feilEl.style.display = 'block'; }
  };
  ...
  for (let b = 0; b < antallBaner; b++) {
    ...
    for (let p = 0; p < 4; p++) {
      const el  = document.getElementById(`manuell-bane${b + 1}-spl${p}`);
      const val = el?.value ?? '';
      if (!val) { visFeil(`Bane ${baneNr} mangler spiller ${p + 1}. Alle 4 plasser må fylles.`); return; }
      if (valgteIds.has(val)) {
        const navn = spillerMap[val]?.navn ?? val;
        visFeil(`${navn} er valgt på flere baner. Hver spiller kan kun spille på én bane.`); return;
      }
      valgteIds.add(val);
      const spiller = spillerMap[val];
      if (!spiller) { visFeil(`Ukjent spiller-ID: ${val}`); return; }
      spillere.push(spiller);
    }
    const gjeldendeMaksPoeng = app.baneOversikt[b]?.maksPoeng ?? app.poengPerKamp;
    nyBaner.push({
      baneNr,
      maksPoeng: gjeldendeMaksPoeng,
      spillere: spillere.map(s => ({ id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING })),
    });
  }

  const nyVenteliste = alleSpillere
    .filter(s => !valgteIds.has(s.id))
    .map(s => ({ id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING }));

  lukkManueltOppsett();
  await _gjennomforNesteRundeKonkurranse(nyBaner, nyVenteliste);
}
window.bekreftManueltOppsett = bekreftManueltOppsett;
...
async function visRundeResultat() {
  ...
  // Sett visResultat-flagg i Firestore ETTER at innholdet er bygget
  // slik at andre brukere har oppdatert baneOversikt når de mottar flagget
  const erSisteRunde = app.runde >= app.maksRunder;
  if (db && app.treningId && !erSisteRunde && getErAdmin()) {
    try {
      const snap = await getDoc(doc(db, SAM.TRENINGER, app.treningId));
      const gjeldendeTeller = snap.data()?.visResultatTeller ?? 0;
      await updateDoc(doc(db, SAM.TRENINGER, app.treningId), { visResultatTeller: gjeldendeTeller + 1 });
    } catch (e) {
      console.warn('[visResultat]', e?.message ?? e);
    }
  }
}
...
async function bekreftNesteRunde() {
  ...
  if (erMix()) {
    await _gjennomforNesteRundeMix();
    return;
  }
  ...
  try {
    ...
    if (app.er6SpillerFormat) {
      await _gjennomfor6SpillerRunde();
      return;
    }
...
async function _skrivNesteRundeFirestore(baneOversikt, nyVenteliste) {
...
async function _gjennomforNesteRundeKonkurranse(baneOversikt, nyVenteliste) {
  if (!db || !app.treningId) { visMelding('Økt ikke aktiv.', 'feil'); return; }
  ...
  try {
    ...
    console.error('[manueltOppsett]', e);
    visMelding(e?.message ?? 'Feil ved neste runde.', 'feil');
    if (!e?.message?.includes('jobber akkurat nå') && !e?.message?.includes('oppdatert av en annen')) {
      await lossTrening();
    }
...
async function _gjennomforNesteRundeMix() {
  lasUI('Starter neste runde…');
  startFailSafe(async () => { await updateDoc(doc(db, SAM.TRENINGER, app.treningId), { laast: false }); });
  ...
  try {
    ...
    console.error('[_gjennomforNesteRundeMix]', e);
    visMelding(e?.message ?? 'Feil ved neste runde.', 'feil');
    if (!e?.message?.includes('jobber akkurat nå') && !e?.message?.includes('oppdatert av en annen')) {
      await lossTrening();
    }
...
async function _gjennomfor6SpillerRunde() {
  const nyRunde  = app.runde + 1;
  const mp       = app.poengPerKamp ?? 15;
...
// ════════════════════════════════════════════════════════
// REDIGER BANER — gjeldende runde
// ════════════════════════════════════════════════════════

function visRedigerBanerModal() {
  // Sjekk om noen kamper allerede har poeng registrert
  const harLagredePoeng = Object.values(kampStatusCache).some(k => k?.ferdig === true);
  krevAdminMedDemo(
    'Rediger baner',
    'Kun administrator kan endre spillerfordelingen.',
    () => {
      if (harLagredePoeng) {
        // Vis advarsel om at eksisterende poeng slettes
        const bekreft = window.confirm(
          '⚠️ Advarsel: Redigering av baner vil slette alle registrerte poeng for denne runden.\n\nEr du sikker på at du vil fortsette?'
        );
        if (!bekreft) return;
      }
      apneRedigerBaner();
...
window.visRedigerBanerModal = visRedigerBanerModal;

// ════════════════════════════════════════════════════════
// REDIGER BANER — Touch drag-and-drop
// Spillere vises gruppert per bane + venteliste.
// Dra en spiller og slipp på en annen → de bytter plass.
// ════════════════════════════════════════════════════════

// Intern tilstand for drag-and-drop redigering
let _redigerState = {
  // [{ gruppe: 'bane'|'vl', baneNr: number|null, spillere: [...] }]
  grupper: [],
  dragIdx: null,   // { gruppeIdx, spillerIdx }
  ghostEl: null,   // det flytende ghost-elementet
  overIdx: null,   // { gruppeIdx, spillerIdx } — der ghost er over
};

function apneRedigerBaner() {
  const feilEl = document.getElementById('rediger-baner-feil');
  if (feilEl) { feilEl.style.display = 'none'; feilEl.textContent = ''; }

  // Bygg intern tilstandsmodell
  _redigerState.grupper = [
    ...(app.baneOversikt ?? []).map(b => ({
      gruppe: 'bane',
      baneNr: b.baneNr,
      spillere: (b.spillere ?? []).map(s => ({ id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING })),
    })),
  ];
  if ((app.venteliste ?? []).length > 0) {
    _redigerState.grupper.push({
      gruppe: 'vl',
      baneNr: null,
      spillere: (app.venteliste ?? []).map(s => ({ id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING })),
    });
  }
  _redigerState.dragIdx  = null;
  _redigerState.ghostEl  = null;
  _redigerState.overIdx  = null;

  _byggRedigerHTML();
  document.getElementById('modal-rediger-baner').style.display = 'flex';
}
window.apneRedigerBaner = apneRedigerBaner;

function _byggRedigerHTML() {
  const innhold = document.getElementById('rediger-baner-innhold');
  if (!innhold) return;

  let html = '';
  _redigerState.grupper.forEach((gr, gi) => {
    const erVL    = gr.gruppe === 'vl';
    const farge   = erVL ? 'var(--orange)' : 'var(--accent)';
    const tittel  = erVL ? '💤 Venteliste' : `Bane ${gr.baneNr}`;
    html += `<div class="rb-gruppe" data-gi="${gi}" style="margin-bottom:20px">
      <div style="font-family:'Bebas Neue',cursive;font-size:20px;letter-spacing:2px;color:${farge};margin-bottom:8px">${tittel}</div>
      <div class="rb-spillerliste" data-gi="${gi}">`;

    gr.spillere.forEach((s, si) => {
      const ini = (s.navn ?? '?').split(' ').map(w => w[0] ?? '').join('').slice(0,2).toUpperCase() || '?';
      html += `<div class="rb-kort" data-gi="${gi}" data-si="${si}"
        style="display:flex;align-items:center;gap:12px;background:var(--navy3);border:1.5px solid var(--border2);border-radius:14px;padding:12px 14px;margin-bottom:8px;cursor:grab;user-select:none;touch-action:none">
        <div style="width:36px;height:36px;border-radius:50%;background:var(--navy2);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:var(--muted2);flex-shrink:0">${escHtml(ini)}</div>
        <div style="flex:1;min-width:0">
          <div style="font-size:17px;font-weight:500;color:var(--white);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escHtml(s.navn ?? 'Ukjent')}</div>
          <div style="font-size:13px;color:var(--muted2);font-family:'DM Mono',monospace">⭐ ${s.rating ?? STARTRATING}</div>
        </div>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" stroke-width="2" style="flex-shrink:0"><circle cx="9" cy="5" r="1" fill="var(--muted)"/><circle cx="9" cy="12" r="1" fill="var(--muted)"/><circle cx="9" cy="19" r="1" fill="var(--muted)"/><circle cx="15" cy="5" r="1" fill="var(--muted)"/><circle cx="15" cy="12" r="1" fill="var(--muted)"/><circle cx="15" cy="19" r="1" fill="var(--muted)"/></svg>
      </div>`;
    });
  ...
  innhold.innerHTML = html;
  _bindRedigerTouchEvents();
}

function _bindRedigerTouchEvents() {
  const kort = document.querySelectorAll('.rb-kort');
  kort.forEach(el => {
    el.addEventListener('touchstart', _redigerTouchStart, { passive: false });
    el.addEventListener('touchmove',  _redigerTouchMove,  { passive: false });
    el.addEventListener('touchend',   _redigerTouchEnd,   { passive: false });
  });
}

function _redigerTouchStart(e) {
  const el  = e.currentTarget;
  const gi  = parseInt(el.dataset.gi, 10);
  const si  = parseInt(el.dataset.si, 10);
  _redigerState.dragIdx = { gi, si };
  ...
  ghost.style.cssText = `
    ...
    width:${rect.width}px;z-index:9999;opacity:.85;
    pointer-events:none;border-color:var(--accent);
    box-shadow:0 8px 32px rgba(0,0,0,.5);transition:none;
    border-radius:14px;background:var(--navy2);cursor:grabbing;
  `;
  ghost._offX = touch.clientX - rect.left;
  ghost._offY = touch.clientY - rect.top;
  document.body.appendChild(ghost);
  _redigerState.ghostEl = ghost;
...
function _redigerTouchMove(e) {
  const ghost = _redigerState.ghostEl;
  if (!ghost) return;
  e.preventDefault();

  const touch = e.touches[0];
  ghost.style.left = (touch.clientX - ghost._offX) + 'px';
  ghost.style.top  = (touch.clientY - ghost._offY) + 'px';

  // Finn hvilken rb-kort vi er over
  ghost.style.display = 'none';
  const underEl = document.elementFromPoint(touch.clientX, touch.clientY);
  ghost.style.display = '';

  const mål = underEl?.closest?.('.rb-kort');

  // Fjern forrige highlight
  document.querySelectorAll('.rb-kort.rb-over').forEach(k => k.classList.remove('rb-over'));

  if (mål && mål !== e.currentTarget) {
    const mGi = parseInt(mål.dataset.gi, 10);
    const mSi = parseInt(mål.dataset.si, 10);
    _redigerState.overIdx = { gi: mGi, si: mSi };
    mål.classList.add('rb-over');
    mål.style.borderColor = 'var(--accent)';
  } else {
    _redigerState.overIdx = null;
  }
}

function _redigerTouchEnd(e) {
  const ghost   = _redigerState.ghostEl;
  const dragIdx = _redigerState.dragIdx;
  const overIdx = _redigerState.overIdx;

  // Gjenopprett original
  document.querySelectorAll('.rb-kort').forEach(k => {
    k.style.opacity = '';
    k.style.borderColor = '';
    k.classList.remove('rb-over');
  });

  if (ghost) { ghost.remove(); _redigerState.ghostEl = null; }

  if (dragIdx && overIdx && !(dragIdx.gi === overIdx.gi && dragIdx.si === overIdx.si)) {
    // Bytt de to spillerne
    const gr1 = _redigerState.grupper[dragIdx.gi];
    const gr2 = _redigerState.grupper[overIdx.gi];
    const s1  = gr1.spillere[dragIdx.si];
    const s2  = gr2.spillere[overIdx.si];
    gr1.spillere[dragIdx.si] = s2;
    gr2.spillere[overIdx.si] = s1;
    _byggRedigerHTML();
  }
...
// Auto-fordeling etter rating (beholder eksisterende gruppestruktur)
function fyllRedigerAutomatisk() {
  const alleSpillere = _redigerState.grupper
    .flatMap(gr => gr.spillere)
    .sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));

  let cursor = 0;
  _redigerState.grupper.forEach(gr => {
    const antall = gr.spillere.length;
    gr.spillere  = alleSpillere.slice(cursor, cursor + antall);
    cursor      += antall;
  });
  _byggRedigerHTML();
}
window.fyllRedigerAutomatisk = fyllRedigerAutomatisk;

// Beholdes for bakoverkompatibilitet — gjør ingenting lenger (tilstand er i _redigerState)
function oppdaterRedigerStatus() {}
window.oppdaterRedigerStatus = oppdaterRedigerStatus;

function lukkRedigerBaner() {
  document.getElementById('modal-rediger-baner').style.display = 'none';
  if (_redigerState.ghostEl) { _redigerState.ghostEl.remove(); _redigerState.ghostEl = null; }
}
window.lukkRedigerBaner = lukkRedigerBaner;

async function bekreftRedigerBaner() {
  if (!db || !app.treningId) { visMelding('Økt ikke aktiv.', 'feil'); return; }

  const feilEl  = document.getElementById('rediger-baner-feil');
  const visFeil = (tekst) => { if (feilEl) { feilEl.textContent = tekst; feilEl.style.display = 'block'; } };

  // Valider: alle bane-grupper skal ha 4 spillere (eller 5 / 2 for spesialformat)
  const baneGrupper = _redigerState.grupper.filter(gr => gr.gruppe === 'bane');
  const vlGruppe    = _redigerState.grupper.find(gr => gr.gruppe === 'vl');

  for (const gr of baneGrupper) {
    const opprinnelig = (app.baneOversikt ?? []).find(b => b.baneNr === gr.baneNr);
    const forventet   = opprinnelig?.spillere?.length ?? 4;
    if (gr.spillere.length !== forventet) {
      visFeil(`Bane ${gr.baneNr} skal ha ${forventet} spillere, men har ${gr.spillere.length}.`);
      return;
    }
  }

  // Bygg nyBaner og nyVenteliste fra tilstand
  const nyBaner = baneGrupper.map(gr => {
    const opprinnelig = (app.baneOversikt ?? []).find(b => b.baneNr === gr.baneNr);
    return {
      ...opprinnelig,
      spillere: gr.spillere.map(s => ({ id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING })),
    };
  });

  const nyVenteliste = (vlGruppe?.spillere ?? []).map(s => ({
    id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING,
  }));

  lasUI('Lagrer…');
  try {
    const kampSnap = await getDocs(query(
      collection(db, SAM.KAMPER),
      where('treningId', '==', app.treningId),
      where('rundeNr',   '==', app.runde)
    ));
    ...
    batch.update(doc(db, SAM.TRENINGER, app.treningId), {
      baneOversikt: nyBaner,
      venteliste:   nyVenteliste,
    });

    nyBaner.forEach(bane =>
      skrivKamper(batch, app.treningId, app.runde, bane.baneNr, bane.spillere,
        bane.erSingel ?? false, bane.erDobbel ?? false)
    );

    await batch.commit();

    app.baneOversikt = nyBaner;
    app.venteliste   = nyVenteliste;
    kampStatusCache  = {};
    lukkRedigerBaner();
    startKampLytter();
    visMelding('Baneoppsett oppdatert!');
  } catch (e) {
...
window.bekreftRedigerBaner = bekreftRedigerBaner;
// ════════════════════════════════════════════════════════
...
async function utforSlettAlleSpillere() {
  ...
  try {
    ...
    // Hent alle spiller-IDer for aktiv klubb
    const spillerSnap = await getDocs(query(collection(db, SAM.SPILLERE), where('klubbId', '==', aktivKlubbId)));
...
function oppdaterAvbrytKnapp() {
  // Avbryt-knappen er fjernet — Avslutt-knappen håndterer begge tilfeller
}
...
async function init() {
  ...
  registrerNavigertHandler(skjerm => {
    if (skjerm === 'baner')    { visBaner(); stoppHjemLytter(); stoppKlubbLytter(); }
    if (skjerm === 'slutt')    { visSluttresultat(); if (!getErAdmin() && !app.treningId) startKlubbLytter(); }
    if (skjerm === 'spillere') oppdaterGlobalLedertavle();
    if (skjerm === 'arkiv')    lastArkiv();
    if (skjerm === 'hjem')     { visHjemStatus(); if (app.treningId) startHjemLytter(); else { stoppHjemLytter(); if (!getErAdmin()) startKlubbLytter(); } }
  });
