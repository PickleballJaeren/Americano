...
function krevAdminMedDemo(tittel, tekst, callback) {
  if (!aktivKlubbId) {
    visMelding('Velg en klubb først.', 'advarsel');
    return;
  }
...
// Dette MÅ skje etter at admin.js har satt sin egen window.krevAdmin, så vi
// bruker en liten timeout for å sikre at modulen er ferdig lastet.
setTimeout(() => { window.krevAdmin = krevAdminMedDemo; }, 0);
...
function lyttPaaSpillere() {
  ...
  spillerLytterAvmeld = onSnapshot(
    ...
    (snap) => {
      ...
      visSpillere();
      // Oppdater siste-deltakere-listen in-place om den er åpen
...
function delLenke() {
  ...
  if (app.treningId) params.set('okt',   app.treningId);
  if (aktivKlubbId)  params.set('klubb', aktivKlubbId);
  const url = location.origin + base + (params.toString() ? '?' + params.toString() : '');

  if (navigator.share) {
    navigator.share({ title: 'Pb Jæren Americano', url }).catch(() => {});
...
function visBaner() {
  const redigerKnapp = document.getElementById('baner-meny-rediger');
  if (redigerKnapp) redigerKnapp.style.display = app.treningId ? 'block' : 'none';
...
// ════════════════════════════════════════════════════════
function toggleBanerMeny() {
  const dd = document.getElementById('baner-meny-dropdown');
  if (!dd) return;
  ...
  if (!open) {
    ...
    setTimeout(() => {
      document.addEventListener('click', lukkBanerMenyUtenfor, { once: true });
    }, 0);
  }
}
function lukkBanerMenyUtenfor(e) {
  const dd = document.getElementById('baner-meny-dropdown');
  const btn = document.getElementById('baner-meny-knapp');
  if (dd && !dd.contains(e.target) && e.target !== btn) lukkBanerMeny();
}
function lukkBanerMeny() {
  const dd = document.getElementById('baner-meny-dropdown');
  if (dd) dd.style.display = 'none';
}
window.toggleBanerMeny = toggleBanerMeny;
window.lukkBanerMeny   = lukkBanerMeny;
  krevAdminMedDemo(
    ...
    () => {
      // Mix-modus: alltid automatisk — vis standard modal
      if (erMix()) {
      ...
      // Skjul manuelt-knappen på siste runde (da går vi rett til resultater)
...
// Lagrer valgt modus fra modal-velg-oppsett: 'auto' | 'manuelt'
...
function velgOppsettModus(modus) {
  ...
  if (modus === 'manuelt') {
    ...
    // Automatisk — samme flyt som før: vis resultater
    visRundeResultat();
  }
}
...
/**
 * Åpner modal for manuelt baneoppsett.
 * Pre-fyller med automatisk fordeling som utgangspunkt.
 */
function apneManueltOppsett() {
  ...
  // Alle spillere i runden (aktive + venteliste)
  const alleSpillere = [
  ...
  // Bygg HTML: én seksjon per bane med dropdown per spillerplass
  let html = '';
  for (let b = 0; b < antallBaner; b++) {
    const baneNr  = b + 1;
    const gjeldende = (app.baneOversikt[b]?.spillere ?? []).map(s => s.id);
    const maksPoeng = app.baneOversikt[b]?.maksPoeng ?? app.poengPerKamp;
  ...
  // Venteliste-seksjon: spillere som ikke spiller denne runden
...
/** Fyller alle baner med auto-forslag (rating-sortert) */
function fyllAutomatiskOppsett() {
  ...
  const aktive      = alleSpillere.slice(0, plasser);
...
/** Oppdaterer venteliste-visning og validering i sanntid */
function oppdaterManuellStatus() {
  ...
  // Venteliste = alle spillere som ikke er valgt
  ...
  // Fargekod dropdowns med duplikater
  for (let b = 0; b < antallBaner; b++) {
    for (let p = 0; p < 4; p++) {
      const el  = document.getElementById(`manuell-bane${b + 1}-spl${p}`);
      const val = el?.value ?? '';
...
/**
 * Leser valg fra manuelt oppsett, validerer og starter neste runde.
 * Bygger baneOversikt fra admin-valg og kaller bekreftNesteRunde-logikken.
 */
async function bekreftManueltOppsett() {
  ...
  for (let b = 0; b < antallBaner; b++) {
    ...
    for (let p = 0; p < 4; p++) {
      ...
      const val = el?.value ?? '';
      if (!val) {
        visOppsettFeil(`Bane ${baneNr} mangler spiller ${p + 1}. Alle 4 plasser må fylles.`);
        return;
      }
      if (valgteIds.has(val)) {
        const navn = spillerMap[val]?.navn ?? val;
        visOppsettFeil(`${navn} er valgt på flere baner. Hver spiller kan kun spille på én bane.`);
        return;
      }
      valgteIds.add(val);
      const spiller = spillerMap[val];
      if (!spiller) { visOppsettFeil(`Ukjent spiller-ID: ${val}`); return; }
      spillere.push(spiller);
    }

    // Behold maksPoeng fra gjeldende bane
  ...
  // Venteliste = spillere som ikke fikk plass
  const nyVenteliste = alleSpillere
    .filter(s => !valgteIds.has(s.id))
    .map(s => ({ id: s.id, navn: s.navn ?? 'Ukjent', rating: s.rating ?? STARTRATING }));

  // Lukk modal og start neste runde med manuelt oppsett
  lukkManueltOppsett();
...
async function bekreftRedigerBaner() {
  ...
  lasUI('Lagrer…');
  try {
    // Slett eksisterende kamper for gjeldende runde og skriv nye
    ...
    // Oppdater baneOversikt og venteliste i treningsdokumentet
    batch.update(doc(db, SAM.TRENINGER, app.treningId), {
      baneOversikt: nyBaner,
      venteliste:   nyVenteliste,
    });

    // Skriv nye kamper for gjeldende runde
    if (erMix()) {
...
async function bekreftNesteRunde() {
  ...
  // Konkurranse: bygg automatisk oppsett basert på forflytninger og kall felles helper
  ...
  try {
    ...
    // ══════════════════════════════════════════
    // STANDARD AMERICANO — beregn automatisk fordeling
    // ══════════════════════════════════════════
...
/**
 * Felles Firestore-skriving for standard konkurranse-runde.
 * Kalles både fra automatisk (bekreftNesteRunde) og manuelt oppsett.
 */
...
/**
 * Kjører neste runde i konkurranse fra et manuelt definert baneoppsett.
 * Låser Firestore, validerer og skriver det nye oppsettet.
 */
...
/** Mix & Match — smart ny lagfordeling (utskilt fra bekreftNesteRunde) */
...
/** 6-spiller rotasjon (utskilt fra bekreftNesteRunde) */
async function _gjennomfor6SpillerRunde() {
  // NB: lås er allerede tatt av bekreftNesteRunde
  const nyRunde  = app.runde + 1;
  const mp       = app.poengPerKamp ?? 15;
...
function oppdaterAvbrytKnapp() {
  const knapp = document.getElementById('baner-meny-avbryt');
  if (!knapp) return;
  const ingenPoeng = Object.values(kampStatusCache).every(k => !k.ferdig);
  knapp.style.display = (app.runde === 1 && ingenPoeng) ? 'block' : 'none';
}
...
async function init() {
  ...
  // Vis alltid tilbake-knappen på oppsett-skjermen så bruker aldri sitter fast
  const tilbakeBoks = document.getElementById('oppsett-tilbake-boks');
  if (tilbakeBoks) tilbakeBoks.style.display = 'flex';
  ...
  // Sjekk om URL inneholder både ?okt= og ?klubb= (delt lenke)
  const urlParams  = new URLSearchParams(location.search);
  const urlKlubbId = urlParams.get('klubb');
  const urlOktId   = urlParams.get('okt');

  if (urlKlubbId && urlOktId && KLUBBER[urlKlubbId]) {
    try {
      aktivKlubbId = urlKlubbId;
      setErAdmin(KLUBBER[urlKlubbId].demo);
      oppdaterKlubbUI();
      const velger = document.getElementById('klubb-velger');
      if (velger) velger.value = urlKlubbId;
      await initEtterKlubbValg();
      return;
    } catch (e) {
      console.warn('[init] Feil ved gjenoppretting fra URL:', e?.message ?? e);
    }
  }

  // Alltid: vis hjemskjerm
  naviger('hjem');
  return;
}
