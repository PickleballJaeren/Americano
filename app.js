...
async function lastSisteDeltakere() {
  ...
  // Bruk cache om fersk nok, men slå alltid sammen med valgtIds
  const naa = Date.now();
  ...
  try {
    ...
    // Slå alltid sammen med valgtIds så spillere valgt via søk ikke forsvinner
    const merged = [...new Set([...app.valgtIds, ...unike])];
...
async function utforAvbrytOkt() {
  ...
  try {
    ...
    visSpillere();
    nullstillSisteDeltakereCache();
    // Åpne siste deltakere-listen med de valgte spillerne
    setTimeout(() => {
