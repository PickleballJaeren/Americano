...
async function lastSisteDeltakere() {
  ...
  const harNyeValgte = [...app.valgtIds].some(id => !gamleIds.includes(id));

  // Bruk cache om fersk nok OG ingen nye valgte siden sist
  if (_sisteDeltakereCache && !harNyeValgte && (naa - _sisteDeltakereCache.hentetMs) < SISTE_DELTAKERE_TTL_MS) {
    const merged = [...new Set([...app.valgtIds, ..._sisteDeltakereCache.spillerIds])];
    _sisteDeltakereCache.spillerIds = merged;
  ...
  // Vis valgte spillere umiddelbart mens Firestore henter
  if (app.valgtIds.size > 0) {
    const forelopig = [...new Set([...app.valgtIds, ...gamleIds])];
    visSisteDeltakere(forelopig);
  }
