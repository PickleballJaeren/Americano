...
function startLyttere() {
  ...
  const l1 = onSnapshot(
    ...
    snap => {
      ...
      if (!getErAdmin() && nyTeller > (_forrigeVisResultatTeller ?? 0)) {
      ...
      // ── Punkt 4: Ny runde startet av admin — naviger til baneoversikt ──
      if (app.runde > forrigeRunde && forrigeRunde !== 0) {
        _forrigeVisResultatTeller = data.visResultatTeller ?? 0; // synkroniser teller ved ny runde
        startKampLytter();
        naviger('baner');
      }
    },
...
async function init() {
  ...
  registrerNavigertHandler(skjerm => {
    if (skjerm === 'baner')    { visBaner(); stoppHjemLytter(); }
    if (skjerm === 'slutt')    visSluttresultat();
    if (skjerm === 'spillere') oppdaterGlobalLedertavle();
    if (skjerm === 'arkiv')    lastArkiv();
    if (skjerm === 'hjem')     { visHjemStatus(); if (app.treningId) startHjemLytter(); else stoppHjemLytter(); }
  });
