...
function apneRedigerBaner() {
  ...
  for (let b = 0; b < antallBaner; b++) {
    ...
    for (let p = 0; p < 4; p++) {
      ...
      html += `<div style="margin-bottom:8px">
        <div style="font-size:13px;text-transform:uppercase;letter-spacing:1px;color:var(--muted2);margin-bottom:4px">Spiller ${p + 1}</div>
        <select id="rediger-bane${baneNr}-spl${p}"
          style="width:100%;padding:12px 14px;background:var(--navy3);border:1px solid var(--border2);border-radius:12px;color:var(--white);font-family:'DM Sans',sans-serif;font-size:17px;appearance:none;-webkit-appearance:none;outline:none"
          onchange="oppdaterRedigerStatus()">
          <option value="">— Velg spiller —</option>
          ${alleSpillere.map(s => `<option value="${escHtml(s.id)}" ${s.id === valgId ? 'selected' : ''}>${escHtml(s.navn ?? 'Ukjent')} (⭐${s.rating ?? 1000})</option>`).join('')}
        </select>
      </div>`;
  ...
  html += `<div style="margin-bottom:16px">
    ...
    <div id="rediger-venteliste-vis" style="font-size:15px;color:var(--muted2);padding:8px 0">Ingen på venteliste</div>
  </div>`;

  document.getElementById('rediger-baner-innhold').innerHTML = html;
  oppdaterRedigerStatus();
...
function fyllRedigerAutomatisk() {
  ...
  for (let b = 0; b < antallBaner; b++) {
    for (let p = 0; p < 4; p++) {
      const el = document.getElementById(`rediger-bane${b + 1}-spl${p}`);
      if (el) el.value = aktive[b * 4 + p]?.id ?? '';
    }
  }
  oppdaterRedigerStatus();
...
function oppdaterRedigerStatus() {
  ...
  const vlSpillere = alleSpillere.filter(s => !valgteIds.has(s.id));
  const vlEl = document.getElementById('rediger-venteliste-vis');
  if (vlEl) {
    vlEl.textContent = vlSpillere.length > 0
      ? vlSpillere.map(s => s.navn ?? 'Ukjent').join(', ')
      : 'Ingen på venteliste';
...
async function bekreftRedigerBaner() {
  ...
  for (let b = 0; b < antallBaner; b++) {
    ...
    for (let p = 0; p < 4; p++) {
      const el  = document.getElementById(`rediger-bane${b + 1}-spl${p}`);
      const val = el?.value ?? '';
      if (!val) { visFeil(`Bane ${baneNr} mangler spiller ${p + 1}.`); return; }
      if (valgteIds.has(val)) {
        visFeil(`${spillerMap[val]?.navn ?? val} er valgt på flere baner.`); return;
      }
      valgteIds.add(val);
      const spiller = spillerMap[val];
      if (!spiller) { visFeil(`Ukjent spiller.`); return; }
      spillere.push(spiller);
    }
    nyBaner.push({
      ...app.baneOversikt[b],
  ...
  try {
    ...
    if (erMix()) {
      skrivMixKamper(batch, app.treningId, app.runde, nyBaner);
    } else {
