...
async function lastSisteDeltakere() {
  ...
  try {
    ...
    // Hent treningSpillere for disse øktene
    // Firestore støtter maks 30 i 'in'-query — deler opp om nødvendig
    const chunks = [];
    ...
    // Legg til spillere fra aktiv økt først (høyest prioritet)
    if (app.treningId) {
      try {
...
function oppdaterSisteDeltakereInPlace() {
  if (!_sisteDeltakereApen || !_sisteDeltakereCache) return;
  visSisteDeltakere(_sisteDeltakereCache.spillerIds);
}
