...
// Bakoverkompatibel const — brukes i bekreftPin()
let ADMIN_PIN = '1234';
...
function byttKlubb(klubbId) {
  ...
  ADMIN_PIN    = KLUBBER[klubbId].pin ?? '';
  erAdmin      = KLUBBER[klubbId].demo; // demo-modus: alltid admin
...
function krevAdmin(tittel, tekst, callback) {
  // Demo-modus eller allerede godkjent — kjør callback direkte
  if (erAdmin || getAktivKlubb()?.demo) {
...
async function lastSisteDeltakere() {
  ...
  // Valgte spillere er alltid med — uavhengig av cache-alder
  const naa = Date.now();
  const gamleIds = _sisteDeltakereCache?.spillerIds ?? [];

  // Slå alltid sammen valgtIds med cache umiddelbart
  const alleredeKjent = [...new Set([...app.valgtIds, ...gamleIds])];

  // Vis umiddelbart fra det vi vet
  if (alleredeKjent.length > 0) {
    visSisteDeltakere(alleredeKjent);
  }

  // Oppdater cache og returner uten Firestore-kall om cachen er fersk nok
  if (_sisteDeltakereCache && (naa - _sisteDeltakereCache.hentetMs) < SISTE_DELTAKERE_TTL_MS) {
    _sisteDeltakereCache.spillerIds = alleredeKjent;
    return;
  }

  const laster = document.getElementById('siste-deltakere-laster');
  if (laster) laster.style.display = 'flex';

  try {
    // Hent siste 10 avsluttede treninger for denne klubben
    ...
    if (treningSnap.empty && !app.treningId) {
      if (laster) laster.style.display = 'none';
      // Vis valgte spillere selv om ingen treninger finnes
      if (app.valgtIds.size > 0) {
        const merged = [...new Set([...app.valgtIds, ...(_sisteDeltakereCache?.spillerIds ?? [])])];
        _sisteDeltakereCache = { spillerIds: merged, hentetMs: Date.now() };
        visSisteDeltakere(merged);
      } else {
        const liste = document.getElementById('siste-deltakere-liste');
        if (liste) liste.innerHTML = '<div style="padding:10px 0;font-size:15px;color:var(--muted2);text-align:center">Ingen tidligere treninger funnet.</div>';
      }
      return;
    }
    ...
    // Hent treningSpillere for avsluttede økter
    const chunks = [];
    for (let i = 0; i < treningIds.length; i += 10) {
      chunks.push(treningIds.slice(i, i + 10));
    }
    ...
    for (const chunk of chunks) {
      ...
      const snap = await getDocs(
        query(collection(db, SAM.TS), where('treningId', 'in', chunk))
      );
      snap.docs.forEach(d => alleTS.push(d.data()));
    }

    // Plukk ut unike spillerIds (maks 20), behold rekkefølge fra nyeste økt
    const sett    = new Set();
    const unike   = [];

    // Legg ALLTID til valgte spillere først — disse skal aldri forsvinne
    for (const id of app.valgtIds) {
      if (!sett.has(id)) { sett.add(id); unike.push(id); }
    }

    // Legg også til spillere som er i eksisterende cache (fra forrige åpning)
    if (_sisteDeltakereCache) {
      for (const id of _sisteDeltakereCache.spillerIds) {
        if (!sett.has(id)) { sett.add(id); unike.push(id); }
      }
    }

    // Legg til spillere fra aktiv pågående økt (Firestore)
    if (app.treningId) {
      try {
        const aktivTS = await getDocs(
          query(collection(db, SAM.TS), where('treningId', '==', app.treningId))
        );
        aktivTS.docs.forEach(d => {
    ...
    // Legg til spillere fra tidligere avsluttede økter
    for (const ts of alleTS) {
      if (unike.length >= 20) break;
      if (ts.spillerId && !sett.has(ts.spillerId)) {
        sett.add(ts.spillerId);
        unike.push(ts.spillerId);
      }
    }

    // Slå alltid sammen: valgtIds + eksisterende cache + Firestore-data
    const tidligereCache = _sisteDeltakereCache?.spillerIds ?? [];
    const merged = [...new Set([...app.valgtIds, ...tidligereCache, ...unike])];
    _sisteDeltakereCache = { spillerIds: merged, hentetMs: Date.now() };
    visSisteDeltakere(merged);
  } catch (e) {
    console.warn('[SisteDeltakere]', e?.message ?? e);
    const liste = document.getElementById('siste-deltakere-liste');
    if (liste) liste.innerHTML = '<div style="padding:10px 0;font-size:15px;color:var(--red2);text-align:center">Kunne ikke laste siste deltakere.</div>';
  } finally {
    const laster = document.getElementById('siste-deltakere-laster');
    if (laster) laster.style.display = 'none';
  }
...
function visAvbrytOktModal() {
  krevAdmin('Avbryt økt', 'Kun administrator kan avbryte økten.', () => {
...
async function utforAvbrytOkt() {
  ...
  try {
    // Slett alle kamp-dokumenter for denne økten
    ...
    // Slett selve treningsdokumentet
