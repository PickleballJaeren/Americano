...
function startLyttere() {
  ...
  const l1 = onSnapshot(
    ...
    snap => {
      ...
      // Kjøres kun for ikke-admin (admin kaller visRundeResultat() selv direkte)
      if (data.visResultat === true && !getErAdmin()) {
        visRundeResultat();
        return;
      }
...
async function visRundeResultat() {
  ...
  if (db && app.treningId && !erSisteRunde && getErAdmin()) {
    try {
      await updateDoc(doc(db, SAM.TRENINGER, app.treningId), { visResultat: true });
    } catch (e) {
